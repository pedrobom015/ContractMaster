...
export default function ChargesTab({ contractId }: ChargesTabProps) {
  ...
  const { data: charges = [], isLoading } = useQuery<ContractCharge[]>({
  ...
  const mutation = useMutation({
    mutationFn: async (data: ContractChargeFormData) => {
      if (editingCharge) {
        return apiRequest(`/api/contract-charges/${editingCharge.id}`, 'PUT', data);
      } else {
        return apiRequest("/api/contract-charges", 'POST', data);
      }
    },
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => apiRequest(`/api/contract-charges/${id}`, 'DELETE'),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/contracts", contractId, "charges"] });
      toast({
        title: "Sucesso",
            ...
            {charges.length === 0 ? (
              ...
              charges.map((charge: ContractCharge) => (
                <TableRow key={charge.id} className="border-border hover:bg-muted/50">
                  ...
                  <TableCell>{charge.dueDate ? new Date(charge.dueDate).toLocaleDateString('pt-BR') : '-'}</TableCell>
                  <TableCell>{formatCurrency(charge.amount)}</TableCell>
                  <TableCell>{getPaymentStatusBadge(charge)}</TableCell>
                  <TableCell>
                    {charge.paymentDate ? new Date(charge.paymentDate).toLocaleDateString('pt-BR') : "-"}
                  </TableCell>
                  <TableCell>

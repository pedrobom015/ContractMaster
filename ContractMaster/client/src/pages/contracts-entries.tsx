...
import type { Contract, ContractServices, ContractBilling } from "@shared/schema";

// Extended contract type with services and billing
interface FullContract extends Contract {
...
export default function ContractsEntries() {
  ...
  // Fetch contracts
  const { data: contracts = [], isLoading } = useQuery<FullContract[]>({
  ...
  const filteredContracts = contracts.filter(contract =>
    String(contract.contractName || "").toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const handleEdit = (contract: FullContract) => {
    ...
    form.reset({
      ...
      groupBatchId: services?.groupBatchId || undefined,
      classId: services?.classId || undefined,
      statusId: services?.statusId || undefined,
      startDate: services?.startDate ? new Date(services.startDate).toISOString().split('T')[0] : "",
      endDate: services?.endDate ? new Date(services.endDate).toISOString().split('T')[0] : "",
      admission: services?.admission ? new Date(services.admission).toISOString().split('T')[0] : "",
      obs: contract.obs || "",
      billingFrequency: billing?.billingFrequency || 1,
      monthInitialBilling: billing?.monthInitialBilling || "",
      yearInitialBilling: billing?.yearInitialBilling || "",
      optPayday: billing?.optPayday || undefined,
      finalGrace: services?.finalGrace ? new Date(services.finalGrace).toISOString().split('T')[0] : "",
      servicesAmount: services?.servicesAmount || undefined,
      firstCharge: billing?.firstCharge || undefined,
      lastCharge: billing?.lastCharge || undefined,
      chargesAmount: billing?.chargesAmount || undefined,
      chargesPaid: billing?.chargesPaid || undefined,
      renewAt: services?.renewAt ? new Date(services.renewAt).toISOString().split('T')[0] : "",
      ownerId: contract.sysUserId || (contract as any).ownerId || undefined,
      collectorId: billing?.collectorId || undefined,
      sellerId: billing?.sellerId || undefined,
      indicatedBy: contract.indicatedBy || undefined,
      regionId: billing?.regionId || undefined,
      serviceOption1: services?.serviceOption1 || "",
      serviceOption2: services?.serviceOption2 || "",
      ownerEmail: "",
      ownerPassword: "",
      createUserAccount: true,
      alives: services?.alives || undefined,
      deceaseds: services?.deceaseds || undefined,
      dependents: services?.dependents || undefined,
    });
                  ...
                  <TableBody>
                    {filteredContracts.map((contract) => (
                      <TableRow key={contract.id} className="border-border/40 hover:bg-muted/20">
                        ...
                        <TableCell>{contract.services?.contractType || '-'}</TableCell>
                        <TableCell>
                          {contract.services?.startDate ? new Date(contract.services.startDate).toLocaleDateString('pt-BR') : '-'}
                        </TableCell>
                        <TableCell>
      ...
      <Dialog open={isCreateDialogOpen || isEditDialogOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="neu-card rounded-2xl max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <Tabs defaultValue="contract-info" className="w-full">
            {/* First Row - Contract Data Tabs (3 normalized tables) */}
            <TabsList className="grid w-full grid-cols-4 neu-flat rounded-xl mb-2">
              <TabsTrigger value="contract-info" className="flex items-center neu-button rounded-lg" data-testid="tab-contract-info">
                <FileText className="w-4 h-4 mr-2" />
                Contrato
              </TabsTrigger>
              <TabsTrigger value="services-info" className="flex items-center neu-button rounded-lg" data-testid="tab-services-info">
                <Settings className="w-4 h-4 mr-2" />
                Serviços
              </TabsTrigger>
              <TabsTrigger value="billing-info" className="flex items-center neu-button rounded-lg" data-testid="tab-billing-info">
                <DollarSign className="w-4 h-4 mr-2" />
                Cobrança
              </TabsTrigger>
              <TabsTrigger value="user-creation" className="flex items-center neu-button rounded-lg" data-testid="tab-user-creation">
                <User className="w-4 h-4 mr-2" />
                Usuário
              </TabsTrigger>
            </TabsList>
            
            {/* Second Row - Child Data Tabs */}
            <TabsList className="grid w-full grid-cols-4 neu-flat rounded-xl">
              <TabsTrigger value="beneficiaries" className="flex items-center neu-button rounded-lg" data-testid="tab-beneficiaries">
                <Users className="w-4 h-4 mr-2" />
                Beneficiários
              </TabsTrigger>
              <TabsTrigger value="charges" className="flex items-center neu-button rounded-lg" data-testid="tab-charges">
                <CreditCard className="w-4 h-4 mr-2" />
                Cobranças
              </TabsTrigger>
              <TabsTrigger value="addresses" className="flex items-center neu-button rounded-lg" data-testid="tab-addresses">
                <MapPin className="w-4 h-4 mr-2" />
                Endereços
              </TabsTrigger>
              <TabsTrigger value="documents" className="flex items-center neu-button rounded-lg" data-testid="tab-documents">
                <FileText className="w-4 h-4 mr-2" />
                Documentos
              </TabsTrigger>
            </TabsList>
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 mt-6">
                ...
                {/* Contract Info Tab - Core contract fields from contracts table */}
                <TabsContent value="contract-info" className="space-y-6">
                  <div className="bg-muted/20 p-4 rounded-xl mb-4">
                    <h3 className="text-lg font-medium mb-1">Dados do Contrato</h3>
                    <p className="text-sm text-muted-foreground">Informações básicas do contrato (tabela contracts)</p>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <FormItem>
                          ...
                          <FormControl>
                            <div className="neu-input rounded-xl">
                              <Input placeholder="Ex: CTR-2025-001" {...field} data-testid="input-contract-number" />
                            </div>
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <div className="neu-input rounded-xl">
                              <Input placeholder="Nome do proprietário" {...field} data-testid="input-contract-name" />
                            </div>
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>Proprietário (ID do usuário)</FormLabel>
                          <FormControl>
                            <div className="neu-input rounded-xl">
                              <Input 
                                type="number" 
                          ...
                          <FormControl>
                            <div className="neu-input rounded-xl">
                              <Input 
                                ...
                                {...field}
                                onChange={(e) => field.onChange(e.target.value ? parseInt(e.target.value) : undefined)}
                                data-testid="input-indicated-by"
                              />
                            </div>
                          </FormControl>
                          <FormMessage />
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <div className="neu-input rounded-xl">
                            <Textarea placeholder="Observações sobre o contrato..." {...field} data-testid="input-obs" />
                          </div>
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                ...
                {/* Services Info Tab - Fields from contract_services table */}
                <TabsContent value="services-info" className="space-y-6">
                  <div className="bg-muted/20 p-4 rounded-xl mb-4">
                    <h3 className="text-lg font-medium mb-1">Dados dos Serviços</h3>
                    <p className="text-sm text-muted-foreground">Informações sobre os serviços contratados (tabela contract_services)</p>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <FormField
                      control={form.control}
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <div className="neu-input rounded-xl">
                              <Input placeholder="Ex: Plano Familiar" {...field} data-testid="input-contract-type" />
                            </div>
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>Grupo/Lote</FormLabel>
                          <FormControl>
                            <div className="neu-input rounded-xl">
                              <Input 
                                type="number" 
                                ...
                                {...field}
                                onChange={(e) => field.onChange(e.target.value ? parseInt(e.target.value) : undefined)}
                                data-testid="input-group-batch-id"
                              />
                            </div>
                          </FormControl>
                          <FormMessage />
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <div className="neu-input rounded-xl">
                              <Input type="date" {...field} data-testid="input-admission" />
                            </div>
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>Fim da Carência</FormLabel>
                          <FormControl>
                            <div className="neu-input rounded-xl">
                              <Input type="date" {...field} data-testid="input-final-grace" />
                            </div>
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <div className="neu-input rounded-xl">
                              <Input type="date" {...field} data-testid="input-renew-at" />
                            </div>
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                          ...
                          <FormControl>
                            <div className="neu-input rounded-xl">
                              <Input 
                                ...
                                data-testid="input-services-amount"
                              />
                            </div>
                          </FormControl>
                          <FormMessage />
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <div className="neu-input rounded-xl">
                              <Input placeholder="Descrição do serviço" {...field} data-testid="input-service-option1" />
                            </div>
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <div className="neu-input rounded-xl">
                              <Input placeholder="Descrição do serviço" {...field} data-testid="input-service-option2" />
                            </div>
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                ...
                {/* Billing Info Tab - Fields from contract_billing table */}
                <TabsContent value="billing-info" className="space-y-6">
                  <div className="bg-muted/20 p-4 rounded-xl mb-4">
                    <h3 className="text-lg font-medium mb-1">Dados de Cobrança</h3>
                    <p className="text-sm text-muted-foreground">Informações sobre cobrança e faturamento (tabela contract_billing)</p>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <FormField
                      control={form.control}
                          ...
                          <FormControl>
                            <div className="neu-input rounded-xl">
                              <Input 
                                ...
                                data-testid="input-billing-frequency"
                              />
                            </div>
                          </FormControl>
                          <FormMessage />
                          ...
                          <FormControl>
                            <div className="neu-input rounded-xl">
                              <Select onValueChange={field.onChange} value={field.value}>
                                <SelectTrigger data-testid="select-month-initial-billing">
                          ...
                          <FormControl>
                            <div className="neu-input rounded-xl">
                              <Input 
                                ...
                                placeholder="2025"
                                {...field}
                                data-testid="input-year-initial-billing"
                              />
                            </div>
                          </FormControl>
                          <FormMessage />
                        ...
                        <FormItem>
                          ...
                          <FormControl>
                            <div className="neu-input rounded-xl">
                              <Input 
                                type="number" 
                                placeholder="Número da primeira cobrança"
                                {...field}
                                onChange={(e) => field.onChange(e.target.value ? parseInt(e.target.value) : undefined)}
                                data-testid="input-first-charge"
                              />
                            </div>
                          </FormControl>
                          <FormMessage />
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>Última Cobrança</FormLabel>
                          <FormControl>
                            <div className="neu-input rounded-xl">
                              <Input 
                                type="number" 
                                placeholder="Número da última cobrança"
                                {...field}
                                onChange={(e) => field.onChange(e.target.value ? parseInt(e.target.value) : undefined)}
                                data-testid="input-last-charge"
                              />
                            </div>
                          </FormControl>
                          <FormMessage />
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>Total de Cobranças</FormLabel>
                          <FormControl>
                            <div className="neu-input rounded-xl">
                              <Input 
                                type="number" 
                                ...
                                {...field}
                                onChange={(e) => field.onChange(e.target.value ? parseInt(e.target.value) : undefined)}
                                data-testid="input-charges-amount"
                              />
                            </div>
                          </FormControl>
                          <FormMessage />
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>Cobranças Pagas</FormLabel>
                          <FormControl>
                            <div className="neu-input rounded-xl">
                              <Input 
                                type="number" 
                                ...
                                {...field}
                                onChange={(e) => field.onChange(e.target.value ? parseInt(e.target.value) : undefined)}
                                data-testid="input-charges-paid"
                              />
                            </div>
                          </FormControl>
                          <FormMessage />

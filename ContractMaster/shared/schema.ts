...
export const sysUsersTable = pgTable("sys_users", {
  ...
  maritalId: integer("marital_id").references(() => generalStatusTable.id),
  frontpageId: integer("frontpage_id"),
  companyId: integer("company_id").references(() => companyTable.id),
  subsidiaryId: integer("subsidiary_id").references(() => subsidiaryTable.id),
  sysUnitId: integer("sys_unit_id").references(() => sysUnitTable.id),
...
export const entityAddressesTable = pgTable("entity_addresses", {
  ...
  entityType: text("entity_type").notNull(), // 'partner'
...
export const partnersTable = pgTable("partners", {
  ...
  currencyId: integer("currency_id").references(() => currencyTable.id),
...
// Clients table - REMOVED: Migrated to use Partners table instead
...
// Billing Control table - temporarily commented out to allow migration
/*
...
// Contracts table - Core contract identity (Account Holder)
...
// Contract Services table - Contracted services details
export const contractServicesTable = pgTable("contract_services", {
  id: integer("id").primaryKey().generatedAlwaysAsIdentity(),
...
// Contract Billing table - Billing and commercial information
export const contractBillingTable = pgTable("contract_billing", {
  id: integer("id").primaryKey().generatedAlwaysAsIdentity(),
  contractId: integer("contract_id").references(() => contractsTable.id).notNull(),
...
// =========================================================================
// FINANCIAL MODULE TABLES - TEMPORARILY DISABLED FOR MIGRATION
// =========================================================================

// Account Types table - TODO: Fix self-reference LSP errors
...
// Chart of Accounts table - TODO: Fix self-reference LSP errors
...
// Cost Centers table - TODO: Fix self-reference LSP errors  
...
// Departments table - TODO: Fix self-reference LSP errors
...
// Projects table - TODO: Re-enable after fixing financial table references
...
// Fiscal Years table - temporarily commented out to allow migration
/*
...
// Fiscal Periods table - temporarily commented out to allow migration
/*
...
export const insertContractServicesSchema = createInsertSchema(contractServicesTable).omit({ 
  id: true, 
  createdAt: true, 
  updatedAt: true,
  deletedAt: true,
  ...
  startDate: z.coerce.date(), // Accepts ISO strings or Date objects
  endDate: z.coerce.date().optional().nullable(), // Optional - accepts ISO strings, Date objects, or undefined/null
  admission: z.coerce.date(), // Required - accepts ISO strings or Date objects
  finalGrace: z.coerce.date().optional().nullable(),
  renewAt: z.coerce.date().optional().nullable(),
});

export const insertContractBillingSchema = createInsertSchema(contractBillingTable).omit({ 
  id: true, 
  createdAt: true, 
  updatedAt: true,
  deletedAt: true,
...
// insertClientSchema - REMOVED: Migrated to use Partners instead
...
export const insertMaritalStatusSchema = createInsertSchema(maritalStatusTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
// Temporarily commented out with fiscalYearsTable
/*
...
// Temporarily commented out with fiscalPeriodsTable
/*
...
// Financial module type definitions - TEMPORARILY DISABLED
...
// Temporarily commented out with billingControlTable
/*
...
export type ContractServices = typeof contractServicesTable.$inferSelect;
export type ContractBilling = typeof contractBillingTable.$inferSelect;
...
// Client type - REMOVED: Migrated to use Partners instead
...
export type MaritalStatus = typeof maritalStatusTable.$inferSelect;
...
export type InsertContractServices = z.infer<typeof insertContractServicesSchema>;
...
// InsertClient type - REMOVED: Migrated to use Partners instead
...
export type InsertMaritalStatus = z.infer<typeof insertMaritalStatusSchema>;
...
// Combined type for frontend display (joins contracts + services + billing)
export type ContractWithDetails = Contract & Partial<ContractServices> & Partial<ContractBilling>;

// Legacy type aliases for compatibility
export type NewContract = InsertContract;
export type NewContractServices = InsertContractServices;
export type NewContractBilling = InsertContractBilling;
...
export type NewClasse = InsertClasse;